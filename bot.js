...
client.on("message", async (message) => {
  ...
  if (conversationState[message.from]) {
    const state = conversationState[message.from];
    // Check the current state of the conversation and respond accordingly
    switch (state) {
      case "awaitingConfirmation":
        if (body.toLowerCase() === "ya") {
          ...
          conversationState[message.from] = null; // Reset the conversation state
        } else {
          await message.reply("Command dibatalkan!");
          conversationState[message.from] = null;
        }
        break;
...
contoh: TIX-???????#close`
        ...
        const lokasiVideo =
          "D:\\xampp\\htdocs\\HelpDesk_waBot\\Media\\hayuk.mp4";
        const dataVideo = fs.readFileSync(lokasiVideo);
        const base64ImageData = dataVideo.toString("base64");
        const kirimMedia = new MessageMedia("video/mp4", base64ImageData);
